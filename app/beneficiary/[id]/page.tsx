    ...
    <div className="flex flex-row gap-4 w-[1280px]">
      <div className="flex flex-col">
                    ...
                    <Listbox variant="flat" aria-label="sub listbox">
                      <ListboxSection>
                        {sidebarItem.subMenu.map((menu) => (
                          <ListboxItem key={menu.key} endContent={menu.icon}>
                            {menu.title}
                          </ListboxItem>
                        ))}
                      </ListboxSection>
        ...
        {sidebarConfig.sidebarItems.slice(2).map((sidebarItem: SidebarItem, index:number) => {
          const { customKey, ...props } = sidebarItem; // Extrae `key` y el resto de las props
          console.log("Esto es customKey :", customKey)
          return <AccordionComponent customKey={customKey} key={index} {...props} />; // Pasa `key` directamente
        })}
      </div>
      <div className="flex flex-col">
        <Card
